...
bool isBipartite(vector<set<int>> G, int src)
{
    ...
    while (!q.empty())
    {
        ...
        for (int v = 0; v < V; ++v)
        {
            ...
            if (G[u].find(v) != G[u].end() && colorArr[v] == -1)
            {
            ...
            else if (G[u].find(v) != G[u].end() && colorArr[v] == colorArr[u])
                return false;
        }
    }
...
//     int V = 4;
//     vector<set<int>> G(V);
//     rep(i, N){set<int> buf; G.push_back(buf);}
//     G[0].insert(1); G[1].insert(0);
//     G[2].insert(1); G[1].insert(2);
//     G[2].insert(3); G[3].insert(2);
//     G[0].insert(3); G[3].insert(0);
